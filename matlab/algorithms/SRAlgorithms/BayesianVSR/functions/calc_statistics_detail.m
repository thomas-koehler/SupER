function stat = calc_statistics_detail(elapsedTime,vid_est,vid_org,vid_bic,opt)
% *************************************************************************
...
%     stat.rmse = mean(stat_rmse(2:opt.nFrames-1)); %sum(stat_rmse)/(opt.nFrames);
%     stat.psnr = mean(stat_psnr(2:opt.nFrames-1)); %sum(stat_psnr)/(opt.nFrames);
%     stat.ssim = mean(stat_ssim(2:opt.nFrames-1)); %sum(stat_ssim)/(opt.nFrames);    
    ...
    if ~isempty(vid_bic)
        ...
        stat_psnr = [];
        stat_ssim = [];
%         for i = 1:length(imgHiBicubic)
        for i = 1:opt.nFrames
            [stat_rmse(i),stat_psnr(i),stat_ssim(i)] = doCalculations(vid_bic{i},vid_org{i},L);
        end
        stat.rmse_bicubic_interpolation = stat_rmse; 
        stat.psnr_bicubic_interpolation = stat_psnr; 
        stat.ssim_bicubic_interpolation = stat_ssim; 
%         stat.rmse_bicubic_interpolation = mean(stat_rmse(2:opt.nFrames-1)); %sum(stat_rmse)/(opt.nFrames);
%         stat.psnr_bicubic_interpolation = mean(stat_psnr(2:opt.nFrames-1)); %sum(stat_psnr)/(opt.nFrames);  
%         stat.ssim_bicubic_interpolation = mean(stat_ssim(2:opt.nFrames-1)); %sum(stat_ssim)/(opt.nFrames);      
    end
